...
class AppListen(AppOp):
    ...
    def project_list(self):
        ...
        for self.row in self.resultCursor.fetchall():
            self.projectsnamelist.append(self.row[0])
        return self.projectsnamelist
    ...
    def collect_pid_list(self, project):
        ...
        print("程序%s的pid结果为：%s" % (self.project, self.pid_lists))
    ...
    def connectpools(self, ports, pids):
        ...
        # 1.内容
        # sscmd = ss -ntp -o state established \'( sport != :%s )\'|grep -E  %s |awk \'{print $4}\'|awk -F \':\' \'{print $(NF-1)\":\" $NF}\'' % (self.port,self.pid)
        ssntpcmd = 'ss -ntp -o state established \\( %s \\)' % self.sportjoin
        print("ssntpcmd is: %s " % ssntpcmd)
    ...
    def import2db(self, table, ipportcolumn, projectcolumn, message):
        """
        MySQL [yed_collect]> alter table listentable add primary key (lipport,projectname);
                MySQL [yed_collect]> show create table listentable\G;
        *************************** 1. row ***************************
               Table: listentable
        Create Table: CREATE TABLE `listentable` (
          `lipport` varchar(24) NOT NULL,
          `projectname` varchar(20) NOT NULL,
          PRIMARY KEY (`lipport`,`projectname`)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8
        1 row in set (0.07 sec)
        MySQL [yed_collect]> alter table pooltable add primary key (conipport,projectname);
                MySQL [yed_collect]> show create table pooltable\G;
        *************************** 1. row ***************************
               Table: pooltable
        Create Table: CREATE TABLE `pooltable` (
        ...
        if len(message) > 0:
            ...
            sql_cmd = "INSERT ignore INTO %s (%s,%s) VALUES %s" % (self.table, self.ipportcolumn, self.projectcolumn, ','.join(self.message))
            print(sql_cmd)
            self.resultCursor.execute(sql_cmd)
            self.DBcon.commit()
        else:
...
def app_l_collect():
    ...
    for project_item in app_listen_con_db_project_list:  # project name list
        todb_ipport_project = []
        todb_conipport_project = []
        # rows, columns = os.popen('stty size', 'r').read().split()
        # print("=" * int(columns) + "\n 1.process project : %s \n" % project_item)
        print("=" * 80 + "\n 1.process project : %s \n" % project_item)
        # print(type(project_item))
        from_db_pid_list = app_listen_instance.collect_pid_list(project_item)  # =>pid列表
        print("2.查询到的pid列表：%s" % from_db_pid_list)
        from_db_ports = app_listen_instance.listenports(from_db_pid_list)  # =>port列表
        if not from_db_ports:
            print("%s 没有在此服务器上运行..." % project_item)
            continue
        # 生成连接池表
        ...
        for port in from_db_ports:
            ...
            for ip in app_listen_instance.get_localhost_ip_list():
                ...
                todb_ipport_project.append(linfo)
        # 生成连接池信息
        for con in collect_con_ipport_list:
            cinfo = "('"+con+"','"+project_item+"')"
            print("cinfo is :%s" % cinfo)
            todb_conipport_project.append(cinfo)
        # print("导入ip列表%s" % todb_ipport_project)
        app_listen_instance.import2db(listentable, listen_ipport_column, projectcolumn, todb_ipport_project)
        # print("导入connect列表%s" % todb_conipport_project)
        app_listen_instance.import2db(connectpooltable, connectpool_ipport_column, projectcolumn, todb_conipport_project)
